...
- **Any call to an external dependency (the LLM provider, in particular) must be wrapped in a typed domain exception, never left to propagate raw.** An unhandled provider-side exception (bad API key, network error, timeout) surfaces to the client as a bare, contentless 500 — useless for debugging and a worse experience than a real error. `Planner.decompose` wraps the LLM call itself in a `try`/`except Exception`, converting any failure into `LLMServiceError` (→ 502, see the Error Handling table) with the provider's own message as detail. This is distinct from `InvalidPlanError`, which means the LLM responded but its plan JSON was malformed.
...
| The LLM provider call itself fails (bad/missing API key, network, rate limit) during planning | `LLMServiceError` → 502 |
| Step exhausts retries | not an HTTP error — recorded on the step (status + `error` message), task proceeds per the failure-recovery policy in `DECISIONS.md` |
